    ...
    >
      ...
      <div className="relative h-40 sm:h-48 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 text-white text-sm">
          ...
          <div className={`flex items-center gap-1 ${availabilityColor[restaurant.availability || 'available']}`}>
            <Clock className="w-4 h-4" />
            <span>
              {(!restaurant.availability || restaurant.availability === 'available') && 'Available now'}
              {restaurant.availability === 'limited' && `${restaurant.waitTime || 15}min wait`}
      ...
      <div className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4 mb-3">
          <div>
            ...
            <p className="text-text-secondary font-inter text-sm">{priceDisplay} · {restaurant.cuisine_types?.join(', ')}</p>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-text-tertiary font-inter text-sm">({restaurant.review_count})</span>
          </div>
        </div>
        ...
        {restaurant.amenities && restaurant.amenities.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {restaurant.amenities.slice(0, 3).map((feature) => (
